...
This repo is on Astro (carried forward from Assignment 1): `astro.config.mjs`
sets `base: "/comp4020-crit5-CongyuLiu09/"`, pages live under `src/pages/`, and
shared markup lives in `src/layouts/`. Use `import.meta.env.BASE_URL` for
internal links rather than hard-coding the repo path.
...
Treat a failed check as evidence that the implementation still needs correction.

Do not ignore a failed check because the website appears correct locally.
...
Keep this deliverable's reflection file (see `reflections/README.md` for the
exact name) focused on:

* the breakthrough that moved the work forward;
* what this work changed about the developer I want to become.
